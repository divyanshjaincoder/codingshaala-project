...
import { Clock, AlertCircle, CheckCircle2, ArrowLeft, ArrowRight } from 'lucide-react';
...
const TestInterface: React.FC<TestInterfaceProps> = ({ onNavigate }) => {
  ...
  const handleSubmitTest = async () => {
    ...
    try {
      ...
      console.log(result)
      const success = await axios.put(`https://codingshaala-backend.onrender.com/api/users/update-user/${loggedInUser._id}`, {
  ...
  const getProgressPercentage = () => (getAnsweredCount() / testQuestions.length) * 100;
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid gap-4">
              <div className="flex items-center gap-3 p-4 rounded-lg bg-muted/50">
                ...
                <div>
                  <p className="font-semibold">Duration: 30 minutes</p>
                  <p className="text-sm text-muted-foreground">Complete within the time limit</p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-4 rounded-lg bg-muted/50">
                <CheckCircle2 className="h-5 w-5 text-success" />
                <div>
                  <p className="font-semibold">Questions: {testQuestions.length}</p>
              ...
              <div className="flex items-center gap-3 p-4 rounded-lg bg-muted/50">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Minimum score required</p>
                </div>
              </div>
            </div>

            <div className="p-4 rounded-lg bg-accent/10 border border-accent/20">
              <h3 className="font-semibold text-accent mb-2">Test Instructions:</h3>
            ...
            <div className="flex gap-3">
              ...
              <Button variant="gradient" onClick={handleStartTest} className="flex-1">
                Start Test
              </Button>
            </div>
          </CardContent>
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className={`font-mono font-bold ${timeLeft < 300 ? 'text-destructive' : 'text-foreground'}`}>
                {formatTime(timeLeft)}
              </p>
            </div>
          </div>
          <div className="mt-4">
            <div className="flex justify-between text-sm mb-2">
              <span>Progress: {getAnsweredCount()}/{testQuestions.length} answered</span>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                      ...
                      {answers[question.id] === index && <CheckCircle2 className="h-4 w-4" />}
                    </div>
                    <span>{option}</span>
                  </div>
                </button>
            ...
            <div className="flex justify-between pt-4">
              ...
              {currentQuestion === testQuestions.length - 1 ? (
                <Button
                  ...
                  disabled={isSubmitting || getAnsweredCount() < testQuestions.length}
                >
